...
PromptMode = Literal["mzn", "pysat", "z3", "maxsat", "asp", "idp"]
...
def get_prompt_path(mode: PromptMode, prompt_type: PromptType = "instructions") -> Path:
    ...
    Args:
        mode: The solver mode ("mzn", "pysat", "z3", "maxsat", "asp", or "idp")
    ...
    """
    # Validate inputs
    if mode not in ("mzn", "pysat", "z3", "maxsat", "asp", "idp"):
        raise ValueError(
            f"Invalid mode: {mode}. Must be one of: mzn, pysat, z3, maxsat, asp, idp"
        )
